...
def grdmask(x, y, t, xf, yf, tf):
    ...
    x0 = numpy.nonzero(numpy.absolute(x[0] - xf) == numpy.min(numpy.absolute(x[0] - xf)))[0]
    y0 = numpy.nonzero(numpy.absolute(y[0] - yf) == numpy.min(numpy.absolute(y[0] - yf)))[0]

    tmask = tf[y0:y0 + length(y), x0:x0 + length(x)]
